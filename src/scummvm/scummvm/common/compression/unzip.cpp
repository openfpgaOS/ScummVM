...
Common::SharedArchiveContents unzOpenCurrentFile (unzFile file
	...
	/* STORE-mode bypass for large entries: skip the full-file copy
	 * into RAM and let the caller read directly from the underlying
	 * stream via a sub-stream view.  Required on memory-constrained
	 * targets (openfpgaOS / Pocket) where a single zip member can be
	 * hundreds of MB.
	 *
	 * For smaller STORE-mode entries we still materialize into RAM so
	 * that byte-at-a-time readers (e.g. readString) don't pay a per-
	 * byte slot-file syscall cost. 16 MB threshold balances the two. */
	if (s->cur_file_info.compression_method == 0 &&
	    s->cur_file_info.compressed_size > 16 * 1024 * 1024) {
		uint32 dataOffset = (uint32)s->cur_file_info_internal.offset_curfile
		                    + SIZEZIPLOCALHEADER + iSizeVar;
		return Common::SharedArchiveContents::bypass(
		    new Common::SafeSeekableSubReadStream(
		        s->_stream,
		        dataOffset,
		        dataOffset + (uint32)s->cur_file_info.compressed_size,
		        DisposeAfterUse::NO));
	}

	if (s->cur_file_info.compression_method == Z_DEFLATED &&
	    s->cur_file_info.uncompressed_size > 16 * 1024 * 1024) {
		uint32 dataOffset = (uint32)s->cur_file_info_internal.offset_curfile
		                    + SIZEZIPLOCALHEADER + iSizeVar;
		Common::SeekableReadStream *compressed =
		    new Common::SafeSeekableSubReadStream(
		        s->_stream,
		        dataOffset,
		        dataOffset + (uint32)s->cur_file_info.compressed_size,
		        DisposeAfterUse::NO);
		Common::SeekableReadStream *inflated =
		    Common::wrapDeflateReadStream(compressed, DisposeAfterUse::YES,
		                                  s->cur_file_info.uncompressed_size);
		if (!inflated)
			return Common::SharedArchiveContents();
		return Common::SharedArchiveContents::bypass(inflated);
	}
