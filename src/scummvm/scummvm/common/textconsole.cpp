...
void warning(const char *s, ...) {
	...
	char buf_output[STRINGBUFLEN + 16];
	va_list va;
	...
	buf_input[STRINGBUFLEN - 1] = '\0';

	if (Common::s_logWatcher)
   		(*Common::s_logWatcher)(LogMessageType::kWarning, 0, 0, buf_input);

	snprintf(buf_output, sizeof(buf_output), "WARNING: %s!\n", buf_input);
	buf_output[sizeof(buf_output) - 1] = '\0';

	if (g_system)
		g_system->logMessage(LogMessageType::kWarning, buf_output);
#else
