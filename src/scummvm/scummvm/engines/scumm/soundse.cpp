...
void SoundSE::initSoundFiles() {
	// If remastered audio is disabled, the whole SE audio map is
	// unused.  Skip the multi-second speech.info parse + xwb header
	// scans -- they're pure dead weight in that case.
	if (!ConfMan.getBool("use_remastered_audio")) {
		warning("SoundSE: skipping SE asset indexing (use_remastered_audio=false)");
		return;
	}
	switch (_vm->_game.id) {
	case GID_MONKEY:
...
Audio::SeekableAudioStream *SoundSE::createSoundStream(Common::SeekableSubReadStream *stream, AudioEntry entry, DisposeAfterUse::Flag disposeAfterUse) {
	...
	case kXWBCodecWMA: {
		// TEMP: revert to silent xWMA path while we isolate whether
		// the engine-freeze comes from our decoder output or somewhere
		// else.  Decoder lives in audio/decoders/wmapro.cpp.
