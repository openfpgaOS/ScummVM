...
#ifdef NONSTANDARD_PORT
	// Handheld intro skip: a left-click (controller A) aborts ONLY the FIRST
	// cutscene of the session (the opening intro).  Once that first cutscene
	// has been entered and then left -- whether skipped with A or watched to
	// the end -- A reverts to a normal left-click, so it no longer skips any
	// later cutscene.  Session-static (not a ScummEngine member, to avoid an
	// ABI/header change); one game runs per boot so this is per-game state.
	{
		static bool s_introSkipArmed = true;   // A may skip the first cutscene
		static bool s_enteredFirstCut = false; // we have been inside it
		if (s_introSkipArmed) {
			if (vm.cutSceneStackPointer != 0) {
				s_enteredFirstCut = true;
				if (_leftBtnPressed & msClicked) {
					_leftBtnPressed &= ~(msClicked | msDown);
					abortCutscene();
					s_introSkipArmed = false;
				}
			} else if (s_enteredFirstCut) {
				s_introSkipArmed = false;   // first cutscene ended; disarm
			}
		}
	}
#endif
