...
void ScummEngine_v5::o5_startScript() {
	...
	if (!_copyProtection) {
		...
		if (_game.id == GID_MONKEY && _game.platform == Common::kPlatformMacintosh && script == 155)
			return;
		// Skip the Monkey Island 2 "Dial-a-Pirate" wheel on boot. The protection
		// *check* is already neutralized by the VAR(490)->VAR(518) redirect in
		// readVar(), but on the DOS-style data the wheel screen (room 108, named
		// "copycrap") is still drawn by global script 130, which the boot script
		// starts and then busy-waits on via isScriptRunning(130). Not starting it
		// makes that wait fall through, so the game boots straight into play -- the
		// same screen-skip the engine already does for MI1 (scripts 152/155).
		//
		// Gated to the boot script (number 1): script 130 is legitimately reused
		// mid-game by a puzzle (the cemetery coffin close-up, room 19 "cu-coffin"),
		// so only the boot-time copy-protection invocation must be suppressed, not
		// every start of script 130. The Macintosh release is excluded as well: it
		// has its own -7873 boot-param path and difficulty/menu use of script 130.
		if (_game.id == GID_MONKEY2 && _game.platform != Common::kPlatformMacintosh &&
			script == 130 && currentScriptSlotIs(1)) {
			// TEMP verification log (remove after confirming on-device): proves the
			// boot-time Dial-a-Pirate launch was intercepted.
			warning("[MI2] Skipping Dial-a-Pirate copy-protection screen (boot startScript 130)");
			return;
		}
	}
