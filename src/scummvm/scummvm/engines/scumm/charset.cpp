...
#ifdef NONSTANDARD_PORT
void *CharsetRenderer::operator new(size_t size) {
	return openfpga_pool_alloc(kOpenFPGAPoolCharsetRenderer, size);
}

void CharsetRenderer::operator delete(void *p) noexcept {
	openfpga_pool_free(kOpenFPGAPoolCharsetRenderer, p);
}

void CharsetRenderer::operator delete(void *p, size_t) noexcept {
	CharsetRenderer::operator delete(p);
}
#endif
