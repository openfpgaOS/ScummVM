...
#endif
#ifdef NONSTANDARD_PORT
		/* The handheld Skip action is an explicit interruption. Hardware
		 * MIDI voices live outside ScummVM's software mixer, so silence
		 * them immediately and let the resumed script start the next cue. */
		openfpga_midi_panic();
		if (_sound && (_game.features & GF_AUDIOTRACKS))
			_sound->stopCD();
#endif
		ss->offs = offs;
		ss->status = ssRunning;
