...
void SoundCD::playCDTrackInternal(int track, int numLoops, int startFrame, int duration) {
	...
	if (!_hasFileBasedCDAudio) {
		...
		if (!stream) {
#ifdef NONSTANDARD_PORT
			// Special-Edition music fallback (openfpga port): MI1's
			// MusicOriginal.xwb is xWMA, which this port can't decode, so the
			// SE stream is null.  Load the classic CD track WAV (track%d.wav
			// bundled in the ISO) and let it play through the SAME file-based
			// handle below -- NOT getAudioCDManager()->play(), whose handle
			// SoundCD never polls/stops, which would leave pollCD/stopCD/
		...
		// duration 0 means "play to the end of the track".  Passing
		// end == start would make makeLoopingAudioStream reject it
		// (start >= end) and delete the stream -- which silenced any cue
		// with a non-zero start and no explicit end (e.g. MI1's title-music
		// reprise at 01:35).  Leave end empty so it falls back to the
		// stream's own length.
		Audio::Timestamp end = duration ? Audio::Timestamp(0, startFrame + duration, 75)
										: Audio::Timestamp(0, 0, 75);
#ifdef NONSTANDARD_PORT
		// Our MS-ADPCM WAV streams report getLength()==0 (ScummVM's wave.cpp
		// leaves MSADPCM samplesPerBlock unparsed), so a start>0 / no-duration
		// cue still can't be turned into a [start,end] sub-range -- end resolves
		// to 0, start>=end, and the stream is dropped.  Fall back to looping the
		// whole track from the start so the cue is at least audible.
		if (startFrame > 0 && !duration && stream->getLength().totalNumberOfFrames() == 0) {
			start = Audio::Timestamp(0, 0, 75);
			end   = Audio::Timestamp(0, 0, 75);
		}
#endif
