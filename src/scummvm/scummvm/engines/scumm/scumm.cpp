...
#include "backend/openfpga_memory.h"
#include <new>
...
#ifdef NONSTANDARD_PORT
static void deleteScummActorObjects(Actor **actors, int numActors) {
	if (!actors)
		return;
...
static Actor **allocScummActorTable(OpenFPGAMemoryPool pool, int numActors) {
	Actor **actors = (Actor **)openfpga_pool_alloc(pool, sizeof(Actor *) * numActors);
	memset(actors, 0, sizeof(Actor *) * numActors);
	return actors;
}

static void freeScummActorTable(OpenFPGAMemoryPool pool, Actor **actors) {
	openfpga_pool_free(pool, actors);
}

template <class T>
static T *newScummActor(ScummEngine *vm, int id) {
	void *p = Actor::operator new(sizeof(T));
	return ::new (p) T(vm, id);
}

#define NEW_SCUMM_ACTOR(type, ...) newScummActor<type>(__VA_ARGS__)
#else
static void deleteScummActorObjects(Actor **actors, int numActors) {
	if (!actors)
		return;
...
#define NEW_SCUMM_ACTOR(type, ...) new type(__VA_ARGS__)
#endif
...
#if !defined(DISABLE_HELP) && !defined(NONSTANDARD_PORT)
...
ScummEngine::~ScummEngine() {
	...
	if (_actors) {
		deleteScummActorObjects(_actors, _numActors);
#ifdef NONSTANDARD_PORT
		freeScummActorTable(kOpenFPGAPoolScummActorTable, _actors);
#else
		delete[] _actors;
#endif
		_actors = nullptr;
...
#ifdef NONSTANDARD_PORT
	freeScummActorTable(kOpenFPGAPoolScummSortedActorTable, _sortedActors);
#else
	delete[] _sortedActors;
#endif
	_sortedActors = nullptr;
...
#ifdef NONSTANDARD_PORT
template <class T>
static T *newCharsetRenderer(ScummEngine *vm) {
	void *p = CharsetRenderer::operator new(sizeof(T));
	return ::new (p) T(vm);
}

template <class T>
static T *newCharsetRenderer(ScummEngine *vm, Common::Language language) {
	void *p = CharsetRenderer::operator new(sizeof(T));
	return ::new (p) T(vm, language);
}

template <class T>
static T *newCharsetRenderer(ScummEngine *vm, const Common::Path &path) {
	void *p = CharsetRenderer::operator new(sizeof(T));
	return ::new (p) T(vm, path);
}

#define NEW_CHARSET_RENDERER(type, ...) newCharsetRenderer<type>(__VA_ARGS__)
#else
#define NEW_CHARSET_RENDERER(type, ...) new type(__VA_ARGS__)
#endif

void ScummEngine::setupCharsetRenderer(const Common::Path &macFontFile) {
	if (_game.version <= 2) {
		if (_game.platform == Common::kPlatformNES)
			_charset = NEW_CHARSET_RENDERER(CharsetRendererNES, this);
		else
			_charset = NEW_CHARSET_RENDERER(CharsetRendererV2, this, _language);
	} else if (_game.version == 3) {
#ifdef USE_RGB_COLOR
		if (_game.platform == Common::kPlatformPCEngine)
			_charset = NEW_CHARSET_RENDERER(CharsetRendererPCE, this);
		else
#endif
		if (_game.platform == Common::kPlatformFMTowns)
			_charset = NEW_CHARSET_RENDERER(CharsetRendererTownsV3, this);
		else if (_game.platform == Common::kPlatformMacintosh && !macFontFile.empty()) {
			_charset = NEW_CHARSET_RENDERER(CharsetRendererMac, this, macFontFile);
		} else
			_charset = NEW_CHARSET_RENDERER(CharsetRendererV3, this);
#ifdef ENABLE_SCUMM_7_8
	} else if (_game.version == 7) {
		CharsetRendererV7 *c7 = NEW_CHARSET_RENDERER(CharsetRendererV7, this);
		_charset = c7;
		createTextRenderer(c7);
	} else if (_game.version == 8) {
		CharsetRendererNut *c8 = NEW_CHARSET_RENDERER(CharsetRendererNut, this);
		_charset = c8;
...
#ifndef DISABLE_TOWNS_DUAL_LAYER_MODE
		if (_game.platform == Common::kPlatformFMTowns)
			_charset = NEW_CHARSET_RENDERER(CharsetRendererTownsClassic, this);
		else
#endif
#endif
			_charset = NEW_CHARSET_RENDERER(CharsetRendererClassic, this);
	}
}

#undef NEW_CHARSET_RENDERER

#ifdef NONSTANDARD_PORT
template <class T>
static T *newCostumeRenderer(ScummEngine *vm) {
	void *p = BaseCostumeRenderer::operator new(sizeof(T));
	return ::new (p) T(vm);
}

template <class T>
static T *newCostumeLoader(ScummEngine *vm) {
	void *p = BaseCostumeLoader::operator new(sizeof(T));
	return ::new (p) T(vm);
}

#define NEW_COSTUME_RENDERER(type, ...) newCostumeRenderer<type>(__VA_ARGS__)
#define NEW_COSTUME_LOADER(type, ...) newCostumeLoader<type>(__VA_ARGS__)
#else
#define NEW_COSTUME_RENDERER(type, ...) new type(__VA_ARGS__)
#define NEW_COSTUME_LOADER(type, ...) new type(__VA_ARGS__)
#endif

void ScummEngine::setupCostumeRenderer() {
	if (_game.features & GF_NEW_COSTUMES) {
		_costumeRenderer = NEW_COSTUME_RENDERER(AkosRenderer, this);
		_costumeLoader = NEW_COSTUME_LOADER(AkosCostumeLoader, this);
	} else if (_game.version == 0) {
		_costumeRenderer = NEW_COSTUME_RENDERER(V0CostumeRenderer, this);
		_costumeLoader = NEW_COSTUME_LOADER(V0CostumeLoader, this);
	} else if (_game.platform == Common::kPlatformNES) {
		_costumeRenderer = NEW_COSTUME_RENDERER(NESCostumeRenderer, this);
		_costumeLoader = NEW_COSTUME_LOADER(NESCostumeLoader, this);
#ifdef USE_RGB_COLOR
	} else if (_game.platform == Common::kPlatformPCEngine) {
		_costumeRenderer = NEW_COSTUME_RENDERER(PCEngineCostumeRenderer, this);
		_costumeLoader = NEW_COSTUME_LOADER(ClassicCostumeLoader, this);
#endif
	} else {
		_costumeRenderer = NEW_COSTUME_RENDERER(ClassicCostumeRenderer, this);
		_costumeLoader = NEW_COSTUME_LOADER(ClassicCostumeLoader, this);
	}
}

#undef NEW_COSTUME_RENDERER
#undef NEW_COSTUME_LOADER
...
void ScummEngine::resetScumm() {
...
#ifdef NONSTANDARD_PORT
	_actors = allocScummActorTable(kOpenFPGAPoolScummActorTable, _numActors);
	_sortedActors = allocScummActorTable(kOpenFPGAPoolScummSortedActorTable, _numActors);
#else
	...
	for (i = 0; i < _numActors; ++i) {
		if (_game.version == 0)
			_actors[i] = NEW_SCUMM_ACTOR(Actor_v0, this, i);
		else if (_game.version <= 2)
			_actors[i] = NEW_SCUMM_ACTOR(Actor_v2, this, i);
		else if (_game.version == 3)
			_actors[i] = NEW_SCUMM_ACTOR(Actor_v3, this, i);
		else if (_game.version >= 7)
			_actors[i] = NEW_SCUMM_ACTOR(Actor_v7, this, i);
		else if (_game.heversion != 0)
			_actors[i] = NEW_SCUMM_ACTOR(ActorHE, this, i);
		else
			_actors[i] = NEW_SCUMM_ACTOR(Actor, this, i);
