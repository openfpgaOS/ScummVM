...
void ScummEngine::fillSavegameLabels() {
	...
	bool isLoomVga = (_game.id == GID_LOOM && _game.version == 4);
		...
		const bool defaultEmptySaveNames = (_menuPage == GUI_PAGE_SAVE);
		Common::String defaultSaveName;
		if (defaultEmptySaveNames) {
			int room = _roomResource ? _roomResource : _currentRoom;
			TimeDate now;
			_system->getTimeAndDate(now);
			if (now.tm_year == 125 && now.tm_mon == 0 && now.tm_mday == 1 && now.tm_hour == 12 && now.tm_min == 0) {
				const uint32 playMinutes = getTotalPlayTime() / (60 * 1000);
				defaultSaveName = Common::String::format("Room %d - %02u:%02u", room, playMinutes / 60, playMinutes % 60);
			} else {
				defaultSaveName = Common::String::format("Room %d - %02d/%02d %02d:%02d",
					room, now.tm_mon + 1, now.tm_mday, now.tm_hour, now.tm_min);
			}
		}
#endif
...
#ifdef NONSTANDARD_PORT
		bool useDefaultSaveName = defaultEmptySaveNames && i == _mainMenuSavegameLabel;
#endif
...
#ifdef NONSTANDARD_PORT
				if (useDefaultSaveName)
					_savegameNames.push_back(Common::String::format("%2d. %s", curSaveSlot, defaultSaveName.c_str()));
				else
#endif
...
#ifdef NONSTANDARD_PORT
				if (useDefaultSaveName)
					_savegameNames.push_back(defaultSaveName);
				else
#endif
...
#ifdef NONSTANDARD_PORT
		const bool editSaveLabel = false;
#else
		const bool editSaveLabel = (_menuPage == GUI_PAGE_SAVE && _mainMenuSavegameLabel > 0);
#endif
		if (editSaveLabel) {
				...
				if (clickedControl < GUI_CTRL_FIRST_SG || clickedControl > GUI_CTRL_LAST_SG ||
					...
					_mainMenuSavegameLabel = clickedControl;
						...
						if (_menuPage == GUI_PAGE_SAVE) {
							fillSavegameLabels();
							const int selectedSaveIndex = _mainMenuSavegameLabel - GUI_CTRL_FIRST_SG;
							if (selectedSaveIndex >= 0 && selectedSaveIndex < (int)_savegameNames.size())
								_internalGUIControls[_mainMenuSavegameLabel].label = _savegameNames[selectedSaveIndex];
						}
#endif
...
#ifdef NONSTANDARD_PORT
		_curDisplayedSaveSlotPage = 0;
		_firstSaveStateOfList = 0;
#endif
		fillSavegameLabels();
		setUpMainMenuControls();
		drawMainMenuControls();
