...
Common::Error ScummMetaEngine::createInstance(OSystem *syst, Engine **engine,
	...
	bool openFPGASkipDetection = ConfMan.hasKey("openfpga_skip_detection") &&
	                             ConfMan.getBool("openfpga_skip_detection");

	// Fetch the list of files in the current directory. openfpgaOS
	// cue/zip launches have already identified the game from game.cfg and
	// expose the ISO tree through SearchMan, not a real /cd directory, so
	// avoid the filesystem enumeration entirely in that mode.
	Common::FSList fslist;
	if (!openFPGASkipDetection) {
	...
	// openfpgaOS fast path: SCUMM's detector MD5-hashes up to 1 MB of
	// every game file just to look the variant up in the table, and
	// over the Pocket's bridge that scan starves the launcher's UART
	// link long enough for phdpd to drop the ACK loop.  When ConfMan
	// flag "openfpga_skip_detection" is set we already know the
	// game (from the per-instance game.cfg) -- look the variant up
	// directly in gameVariantsTable and skip the file scan.  Borrow
	// a real MD5 from md5table for the matching gameid so the engine's
	// "unknown game version" warning stays quiet.
	if (openFPGASkipDetection) {
		Common::String wantVariant = ConfMan.hasKey("extra")
		                              ? ConfMan.get("extra")
		                              : Common::String();
		for (const GameSettings *g = gameVariantsTable; g->gameid; ++g) {
			if (strcmp(g->gameid, gameid) != 0) continue;
			if (!wantVariant.empty() && g->variant &&
			    strcmp(g->variant, wantVariant.c_str()) != 0)
				continue;

			Common::String borrowedMD5;
			for (int i = 0; md5table[i].gameid; ++i) {
				if (strcmp(md5table[i].gameid, gameid) != 0) continue;
				if (g->variant && md5table[i].variant &&
				    strcmp(md5table[i].variant, g->variant) != 0)
					continue;
				borrowedMD5 = md5table[i].md5;
				break;
			}
			...
			/* Add MDT_MIDI + MDT_PREFER_GM to games that already expose
			 * a MIDI backend so our openfpga GM driver gets matched by
			 * MidiDriver::detectDevice().  Preserve MDT_NONE for CD-audio
			 * only games like Loom VGA; forcing MIDI there prevents the
			 * engine from taking its AudioCD path. */
			if (r.game.midi != MDT_NONE)
				r.game.midi |= MDT_MIDI | MDT_PREFER_GM;
			r.language    = ConfMan.hasKey("language")
			                  ? Common::parseLanguage(ConfMan.get("language"))
			                  : Common::UNK_LANG;
			r.md5         = borrowedMD5;
			r.extra       = g->variant ? g->variant : "";

			/* Previously stripped GF_AUDIOTRACKS for SE variants to
			 * coax iMUSE/MIDI, but the SE PAK's classic LFL files
			 * store music as SOUN type-2 CD-track refs without a MIDI
			 * fallback, so stripping the flag just confuses the
			 * splash-screen sequencer and freezes the engine.  Leave
			 * the flag intact; SE music is silent until xWMA decode
			 * actually produces samples. */

			/* Pick a GameFilenamePattern by gameid+(optional variant).
			 * Preferred: an entry whose canonical first filename
			 * (room 0 / disk 0) actually exists in fslist.  Fallback:
			 * the first entry matching gameid, regardless of fslist
			 * -- ScummVM still needs *a* valid pattern or it errors
			 * with "kGenUnchanged used with unsupported platform".
			 * (CD ISOs often store game data in a subdirectory, so
			 * fslist may not contain the .000 file at the mount root
			 * even though the engine will still find it later.) */
			r.fp.pattern   = nullptr;
			r.fp.genMethod = kGenUnchanged;
			/* Two-stage pick:
			 *  1. If the per-instance .cfg supplied a variant (e.g. "SE"),
			 *     pick the first gameFilenamesTable entry whose variant
			 *     matches.  This is the authoritative choice -- the variant
			 *     uniquely identifies the file layout.
			 *  2. Otherwise (no variant given), probe fslist for an existing
			 *     file matching each entry's pattern, like ScummVM's detector
			 *     does.  Fall back to the first entry for the gameid if
			 *     nothing probes successfully (ISO subdir / empty FSNode dir
			 *     cases).
			 */
			const GameFilenamePattern *fallback = nullptr;
			if (!wantVariant.empty()) {
				for (const GameFilenamePattern *gfp = gameFilenamesTable;
				     gfp->gameid; ++gfp) {
					if (strcmp(gfp->gameid, gameid) != 0) continue;
					if (gfp->variant &&
					    strcmp(gfp->variant, wantVariant.c_str()) == 0) {
						r.fp.pattern   = gfp->pattern;
						r.fp.genMethod = gfp->genMethod;
						break;
					}
				}
			}
			if (!r.fp.pattern) {
				for (const GameFilenamePattern *gfp = gameFilenamesTable;
				     gfp->gameid; ++gfp) {
					if (strcmp(gfp->gameid, gameid) != 0) continue;
					if (!fallback) fallback = gfp;
					Common::String probe =
					    generateFilenameForDetection(gfp->pattern,
					                                 gfp->genMethod,
					                                 gfp->platform);
					if (probe.empty()) continue;
					Common::FSNode hit;
					if (searchFSNode(fslist, probe, hit)) {
						r.fp.pattern   = gfp->pattern;
						r.fp.genMethod = gfp->genMethod;
						break;
					}
					/* fslist is empty for ZIP-mode (/cd not mounted),
					 * but SearchMan has the zip registered by main.cpp
					 * before createInstance.  Probe via Common::File
					 * which consults SearchMan. */
					if (Common::File::exists(Common::Path(probe))) {
						r.fp.pattern   = gfp->pattern;
						r.fp.genMethod = gfp->genMethod;
						break;
					}
			...
			if (!r.fp.pattern && fallback) {
				r.fp.pattern   = fallback->pattern;
				r.fp.genMethod = fallback->genMethod;
			}

			results.push_back(r);

			/* Pin the variant in ConfMan so the post-detection
			 * setAndFlush("extra", ...) at line ~498 sees the
			 * key already present and short-circuits.  Without
			 * this guard ScummVM tries to write scummvm.ini to
			 * a nonvolatile slot, which stalls the launcher's
			 * UART pump on the Pocket. */
			if (!ConfMan.hasKey("extra"))
				ConfMan.set("extra", g->variant ? g->variant : "");
			break;
		}
	}
	...
	// file transparently. openfpgaOS launches use fixed per-game cfg and
	// config persistence is disabled, so skip this heap-heavy migration path.
	if (!openFPGASkipDetection)
...
int ScummMetaEngine::getAutosaveSlot() const {
#ifdef NONSTANDARD_PORT
	return -1;
#else
	return MetaEngine::getAutosaveSlot();
#endif
}

int ScummMetaEngine::getMaximumSaveSlot() const {
#ifdef NONSTANDARD_PORT
	return 8;
#else
	return 99;
