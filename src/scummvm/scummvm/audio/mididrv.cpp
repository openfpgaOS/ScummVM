...
#ifdef NONSTANDARD_PORT
namespace {
bool isOpenFPGAMidiIdentifier(const Common::String &identifier) {
	return identifier.equalsIgnoreCase("openfpga") ||
	       identifier.equalsIgnoreCase("openfpgaOS MIDI");
}
} // namespace
#endif
...
#ifdef NONSTANDARD_PORT
	if (openfpga_midi_is_device_handle(handle))
		return MT_GM;
#endif
...
#ifdef NONSTANDARD_PORT
	if (openfpga_midi_is_device_handle(handle)) {
		if (type == kDriverName || type == kDeviceName)
			return Common::String("openfpgaOS MIDI");
		return Common::String("openfpga");
	}
#endif
...
#ifdef NONSTANDARD_PORT
	if ((flags & MDT_MIDI) &&
	    (selDevStr.equalsIgnoreCase("auto") || selDevStr.equalsIgnoreCase("fluidsynth") ||
	     isOpenFPGAMidiIdentifier(selDevStr))) {
		_forceTypeMT32 = false;
		return openfpga_midi_device_handle();
	}
#endif
...
#ifdef NONSTANDARD_PORT
	if (openfpga_midi_is_device_handle(handle))
		return openfpga_midi_create_driver();
#endif
...
#ifdef NONSTANDARD_PORT
	if (isOpenFPGAMidiIdentifier(identifier))
		return openfpga_midi_device_handle();
#endif
