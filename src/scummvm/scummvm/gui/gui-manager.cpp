...
GuiManager::GuiManager() : CommandSender(nullptr), _redrawStatus(kRedrawDisabled), _stateIsSaved(false),
	...
	// openfpgaOS is a direct-launch backend with its own controller
	// event mapping. Keep the GUI singleton usable for keymap callers,
	// but do not load launcher icons or themes on the limited runtime heap.
	return;
#endif
